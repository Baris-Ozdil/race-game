...
public class audio : MonoBehaviour
{
    ...
    private void Start()
    {
        if (this.gameObject.tag == "Player")
        {
            ...
            InputManager = player.GetComponent<InPutManger>();
            isPlayer = true;
        }
        else if(this.gameObject.tag == "AiCar")
        {
             AI = gameObject.GetComponent<aiControler>();
            aiP = gameObject.GetComponent<aiInput>();
        }
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (m_StartedSound)
        {
            if (this.gameObject.tag == "Player")
            {
                ...
                float pitch = ULerp(lowPitchMin, lowPitchMax, cont.enginRPM / cont.maxRPM);
                ...
                else
                {
                    ...
                    accFade = Mathf.Abs((InputManager.vertical > 0 && !cont.test) ? InputManager.vertical : 0);

                    float decFade = 1 - accFade;

                    // get the high fade value based on the cars revs
                    float highFade = Mathf.InverseLerp(0.2f, 0.8f, cont.enginRPM / 10000);
            ...
            {
                // The pitch is interpolated between the min and max values, according to the car's revs.
                float pitch = ULerp(lowPitchMin, lowPitchMax, AI.enginRPM / AI.maxRPM);
                ...
                else
                {
                    ...
                    accFade = Mathf.Abs((aiP.vertical > 0 && !AI.test) ? aiP.vertical : 0);

                    float decFade = 1 - accFade;

                    // get the high fade value based on the cars revs
                    float highFade = Mathf.InverseLerp(0.2f, 0.8f, AI.enginRPM / 10000);
                    float lowFade = 1 - highFade;
